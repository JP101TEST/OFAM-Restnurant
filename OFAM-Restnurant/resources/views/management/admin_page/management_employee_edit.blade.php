...
$employee_data = [
    $employee[0]->employees_id,
    $employee[0]->employees_password,
    $employee[0]->first_name,
    $employee[0]->last_name,
    $employee[0]->house_number,
    $employee[0]->road,
    $employee[0]->sub_district,
    $employee[0]->district,
    $employee[0]->province,
    $employee[0]->postal_code,
    $employee[0]->employees_phone,
];
...
if (session('errorEmployee')) {
    ...
    for ($i = 0; $i < sizeof($employee_data); $i++) {
        if ($employee_data[$i] != $errorEmployee[$i]) {
            $errorEmployee[$i] = $employee_data[$i];
        }
    }
} else {
    $errorEmployee = $employee_data;
}
...
<body>
    ...
    <div class="container">
        ...
        <div class="row mt-3">
            <div class="col-lg-12">
                <br>
                <h4 class="text-center">แก้ไขพนักงาน</h4>
                <div class="mb-3 container">
                    <p>แก้ไข:<a class="text_red">(สามารถเว้นว่างได้กรณีที่ต้องการข้อมูลเป็นแบบเดิมโดยที่ลืมค่าเก่าหลังแก้ไข)</a></p>
                </div>
                <form method="post" action="{{ route('management.admin.employee.edit.postData',['employee_id' => $employee[0]->employees_id] ) }}" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            @endif
                            <input type="text" class="form-control error-input" id="formGroupExampleInput1" name="employeeId" value="{{ $errorEmployee ? $errorEmployee[0] : NULL}}" placeholder="กรอกรหัส" oninput="checkInputs()" disabled>
                        </div>
                        <div class="col-md-6 mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        @endif
                        <input type="file" class="form-control error-input" id="formGroupExampleInput5" name="employeeImage" accept="image/*" placeholder="สามารถเว้นว่างได้กรณีที่ไม่ต้องการเปลี่ยนรูป" oninput="checkInputs()">
                    </div>
...
<script>
    function checkInputs() {
        ...
        if (phone == false || postalCode == false) {
