...
<body>
    ...
    <div class="container">
        ...
        <div class="row mt-3">
            <div class="col-lg-12">
                <br>
                <h4 class="text-center">เพิ่มหมวดหมู่</h4>
                <div class="mb-3 container">
                    <p>เพิ่ม:</p>
                </div>
                <form method="post" action="{{ route('management.admin.food.category.add.postData') }}" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-3">
                        <img class="icon-size-no-brightness spade-bar" src="{{ asset('images/dinner-table.png') }}" alt="">
                        <label for="formGroupExampleInput" class="form-label">ชื่อหมวดหมู่:</label>
                        @if (session('errorCategoryName'))
                        <p class="text-center text-light bg-danger">{{ session('errorCategoryName') }}</p>
                        @endif
                        <input type="text" class="form-control error-input" id="formGroupExampleInput" name="categoryName" placeholder="กรอกชื่อหมวดหมู่" oninput="checkInputs()">
                        <!--<p id="formGroupExampleOutput"></p>-->
                    </div>

                    {{ session()->forget(['errorCategoryName']) }}
                    <div class="col-lg-12 d-flex justify-content-between">
...
<script>
    function checkInputs() {
        const categoryName = document.getElementById('formGroupExampleInput');
